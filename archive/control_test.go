...
func encode(t *testing.T, data encodable) *atomic.Buffer {
	...
	return atomic.NewBufferSlice(buf.Bytes())
}
...
func newTestSub(image aeron.Image) *aeron.Subscription {
	...
	sub := aeron.NewSubscription(nil, "", 1, 2, 3)
...
func newTestHeader() *logbuffer.Header {
	buffer := atomic.NewBufferSlice(make([]byte, logbuffer.DataFrameHeader_Length))
	buffer.PutUInt8(logbuffer.DataFrameHeader_FlagsFieldOffset, 0xc0) // unfragmented
